...
interface EffortCardProps {
  ...
  title: string;
  subtitle?: React.ReactNode;
  animationKey: string; // Add this to trigger animation
}

const EffortCard: React.FC<EffortCardProps> = ({ number, bgColor, icon, title, subtitle, animationKey }) => {
  return (
    <div
      className="effort-card relative flex flex-col items-center gap-5 md:gap-[30px] w-full "
      style={{
    ...
    >
      ...
      <div key={animationKey} className="animate-content flex flex-col items-center gap-5 md:gap-[30px]">
        <div
          className={`min-w-[58px] min-h-[58px] md:min-h-20 p-3.5 md:p-5 rounded-xl md:rounded-[20px] inline-flex items-center justify-center`}
          style={{ backgroundColor: bgColor }}
        >
